...
discord_bot.on('ready', () => {
	...
	discord_bot.user.setActivity('$vacbot', {
		type: "LISTENING"
	})
}) // bot logged in
...
async function addDistrict(id, name) {
	...
	let exists = await database.collection('districts').countDocuments({
		code: id
	})
	if (!exists) {
		await database.collection('districts').insertOne({
			code: id,
			name: name
		})
	}
...
async function addUser(id, name, code, district_name, age) {
	const database = dbClient.db(db_name)
	let exists = await database.collection('users').countDocuments({
		id: id
	})
	if (!exists) {
		...
		const dis = await database.collection('users').findOne({
			id: id
		})
...
async function removeDistrict(id) {
	const database = dbClient.db(db_name)
	let number = await database.collection('users').countDocuments({
		code: id
	})
	if (number < 2) {
		await database.collection('districts').deleteOne({
			code: id
		})
	}
		...
		if (resp.statusCode == 200) {
			...
			resp.on('end', async () => {
				...
				let regMsg = new Discord.MessageEmbed()
					.addFields({
						name: "To register,visit",
						value: "https://www.cowin.gov.in/home"
					})
