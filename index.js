require('dotenv').config() //env variables aaaaaa step 2
    //vacbot
...
async function showUser(id) {
    ...
    if (exists) {
        ...
        let userMessage = new Discord.MessageEmbed()
            .setColor('#44c544')
            .setTitle('User Data')
            .addFields({ name: 'Name', value: userData.name }, { name: 'Age', value: userData.age }, { name: 'District code', value: userData.code }, { name: 'District Name', value: userData.district }, );
        const fetchedUser = await discord_bot.users.fetch(id).catch(() => console.log('could not find user'));
